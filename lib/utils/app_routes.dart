class AppRoutes {
  static const String home = "/";
  static const String monthListPage = "/month-list-page";
  static const String categoryListPage = "/category-list-page";
  static const String revenueListPage = "/revenue-list-page";
  static const String detailPage = '/detail-page';
  static const String detailBillPage = '/detail-bill-page';
  static const String statsPage = '/stats-page';
  static const String addBillPage = '/add-bill-page';
  static const String addCategoryPage = '/add-category-page';
  static const String addRevenuePage = '/add-revenue-page';
  static const String chooseCategoryPage = '/choose-category-page';
  static const String choosePaymentPage = '/choose-payment-page';
  static const String profilePage = '/profile-page';
}